...
var testCreateUpdateWithFilter = function () {
    fs.copySync(functionsSource, tmpDir + "/functions/index.js");
    return new Promise(function (resolve) {
        exec(`${localFirebase} deploy --only functions:nested,functions:httpsAction --project=${projectId}`, { cwd: tmpDir }, function (err, stdout) {
            console.log(stdout);
            expect(err).to.be.null;
...
var testUnknownFilter = function () {
    return new Promise(function (resolve) {
        exec("> functions/index.js &&" +
            `${localFirebase} deploy --only functions:unknownFilter --project=${projectId}`, { cwd: tmpDir }, function (err, stdout) {
            console.log(stdout);
...
var testFunctionsTrigger = function () {
    ...
    var checkScheduleAction = triggerSchedule("firebase-schedule-pubsubScheduleAction-us-central1").then(function () {
        return true;
    });
    return Promise.all([
        checkDbAction,
...
var main = function () {
    preTest()
        ...
        .then(function () {
        console.log(clc.green("\u2713 Test passed: threw warning when passing filter with unknown identifier"));
    })
        .catch(function (err) {
...
main();
//# sourceMappingURL=test-functions-deploy.js.map